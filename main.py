...
def write_remaining_states():
    remaining_states_dataframe = pandas.DataFrame({"States to Study": remaining_states})
    remaining_states_dataframe.to_csv("states_to_study.csv")


def display_state_name(row_data):
    state_name = row_data.state.item()
    x_cor = row_data.x.item()
    y_cor = row_data.y.item()
    pen.setpos(x_cor, y_cor)
